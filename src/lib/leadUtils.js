...
export const getPhoneVariations = (phone) => {
    ...
    // With '9' for mobile numbers
    if (normalized.startsWith('55') && normalized.length === 12) { // 55 + 10 digits (no 9)
        const ddd = normalized.substring(2, 4);
        const number = normalized.substring(4);
        variations.add(`55${ddd}9${number}`);
    }
    
    // Without '9' for mobile numbers
    if (normalized.startsWith('55') && normalized.length === 13 && normalized.charAt(4) === '9') { // 55 + 11 digits (with 9)
        const ddd = normalized.substring(2, 4);
